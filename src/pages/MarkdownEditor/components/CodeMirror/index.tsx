...
export default class ReactCodeMirror extends Component<ICodeMirror, ICodeMirrorState> {
  ...
  public componentDidMount() {
    ...
    const { options, defineMode } = this.props;
    if (defineMode) {
      if (defineMode.name && defineMode.fn) {
        cm.defineMode(defineMode.name, defineMode.fn);
      }
    }

    // const editorOption = { tabSize: 2, lineNumbers: false, ...options, mode: 'markdown' };
    // 生成codemirror实例
    this.editor = cm.fromTextArea(this.textarea, options) as CodeMirror.EditorFromTextArea;
